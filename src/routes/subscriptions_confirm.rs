...
impl TryFrom<Parameters> for SubscriberToken {
    ...
    fn try_from(value: Parameters) -> Result<Self, Self::Error> {
        let token = SubscriberToken::parse(value.subscription_token)?;
        Ok(token)
    }
}
...
pub async fn confirm(parameters: web::Query<Parameters>, pool: web::Data<PgPool>) -> HttpResponse {
    let token = match parameters.0.try_into() {
        Ok(t) => t,
        Err(_) => return HttpResponse::BadRequest().finish(),
    };
    let id = match get_subscriber_id_from_token(&pool, &token).await {
        Ok(id) => id,
